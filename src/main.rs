...
fn handle_connection(mut stream: TcpStream) {
    ...
    let (status_line, filename) = match &request_line[..] {
        ...
        "GET /list-files HTTP/1.1\r\n" => {
            // Get the list of files and send as JSON
        ...
        _ if request_line.starts_with("GET /download/") => {
            ...
            match fs::read(file_to_download) {
                Ok(contents) => {
...
fn list_files() -> String {
    // List files in the current directory and return as a JSON string
...
fn handle_file_upload(body: Vec<u8>, stream: &mut TcpStream) {
    ...
    if let Some(pos) = body.windows(boundary.len()).position(|window| window == boundary) {
        ...
        let filename = headers.lines()
            ...
            .unwrap_or("uploaded_file"); // Default filename if none provided

        fs::write(filename, file_data).expect("Unable to write file"); // Save the uploaded file
